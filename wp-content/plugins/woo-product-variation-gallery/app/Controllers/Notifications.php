...
class Notifications {
	...
	public function wc_requirement_notice() {
		if ( ! rtwpvg()->is_wc_active() ) {
			...
			$message = wp_kses( __( "<strong>WooCommerce Variation images gallery</strong> is an add-on of ", 'woo-product-variation-gallery' ), array( 'strong' => array() ) );
	...
	public function wc_version_requirement_notice() {
		if ( rtwpvg()->is_wc_active() && ! rtwpvg()->is_valid_wc_version() ) {
			$class   = 'notice notice-error';
			$message = sprintf( esc_html__( "Currently, you are using older version of WooCommerce. It's recommended to use latest version of WooCommerce to work with %s.", 'woo-product-variation-gallery' ), esc_html__( 'WooCommerce Variation images gallery', 'woo-product-variation-gallery' ) );
