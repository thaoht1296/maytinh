 ...
 *
 */
abstract class SP_LW_Framework_Options extends SP_LW_Framework_Abstract {
	...
	public function element_debug() {
		...
		if( ( isset( $this->field['debug'] ) && $this->field['debug'] === true ) || ( defined( 'SP_OPTIONS_DEBUG' ) && SP_OPTIONS_DEBUG ) ) {
			...
			$out .= "<pre>";
			$out .= "<strong>". __( 'CONFIG', 'location-weather' ) .":</strong>";
			$out .= "\n";
			ob_start();
			var_export( $this->field );
			$out .= htmlspecialchars( ob_get_clean() );
			$out .= "\n\n";
			$out .= "<strong>". __( 'USAGE', 'location-weather' ) .":</strong>";
			$out .= "\n";
			$out .= ( isset( $this->field['id'] ) ) ? "sp_get_option( '". $this->field['id'] ."' );" : '';

			if( ! empty( $value ) ) {
				$out .= "\n\n";
				$out .= "<strong>". __( 'VALUE', 'location-weather' ) .":</strong>";
				$out .= "\n";
				ob_start();
				var_export( $value );
				$out .= htmlspecialchars( ob_get_clean() );
		...
		if( ( isset( $this->field['debug_light'] ) && $this->field['debug_light'] === true ) || ( defined( 'SP_OPTIONS_DEBUG_LIGHT' ) && SP_OPTIONS_DEBUG_LIGHT ) ) {
			...
			$out .= ( isset( $this->field['id'] ) ) ? "sp_get_option( '". $this->field['id'] ."' );" : '';
			$out .= "\n";
			$out .= "<strong>". __( 'ID', 'location-weather' ) .":</strong>";
			$out .= "\n";
		...
		if ( is_array( $this->multilang ) ) {
			...
			foreach ( $this->multilang['languages'] as $key => $val ) {
				...
				if( $key != $this->multilang['current'] ) {
					...
					$class = 'SP_LW_Framework_Option_' . $this->field['type'];
			...
			$out .= '</fieldset>';
			$out .= '<p class="sp-text-desc">'. sprintf( __( 'You are editing language: ( <strong>%s</strong> )', 'location-weather' ), $this->multilang['current'] ) .'</p>';
