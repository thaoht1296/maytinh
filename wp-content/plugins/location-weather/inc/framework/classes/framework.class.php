 ...
 *
 */
class SP_LW_Framework extends SP_LW_Framework_Abstract {
	...
	public function validate_save( $request ) {
		...
		if ( isset( $request['resetall'] ) ) {
			$add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored.', 'location-weather' ), 'updated' );
			return;
		}
		...
		if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
			...
			$add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored for only this section.', 'location-weather' ), 'updated' );
		}
	...
	public function admin_page() {
		...
		echo ( $this->settings['ajax_save'] ) ? '<span id="sp-save-ajax">'. esc_html__( 'Settings saved.', 'location-weather' ) .'</span>' : '';

		submit_button( esc_html__( 'Save Changes', 'location-weather' ), 'primary sp-save', 'save', false, array( 'data-save' => esc_html__( 'Saving...', 'location-weather' ) ) );
		submit_button( esc_html__( 'Restore', 'location-weather' ), 'secondary sp-restore sp-reset-confirm', $this->unique .'[reset]', false );

		if( $this->settings['show_reset_all'] ) {
			submit_button( esc_html__( 'Reset All Options', 'location-weather' ), 'secondary sp-restore sp-warning-primary sp-reset-confirm', $this->unique . '[resetall]' , false );
		}

		echo '</fieldset>';
		//echo ( empty( $has_nav ) ) ? '<a href="#" class="sp-expand-all"><i class="fa fa-eye-slash"></i> '. esc_html__( 'show all options', 'location-weather' ) .'</a>' : '';
